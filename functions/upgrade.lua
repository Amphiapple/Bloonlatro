...
local function is_upgradable_card(card)
    ...
    if base and JokerTable[base] then
        return card.ability.tower_info.tier ~= 6
    end
    return false
end
...
function BTD.get_tower_upgrade(card, paragon, t5, category, path, tiers)
    ...
    if start_tier >= 5 and tiers > 0 then
        return nil
    elseif t5 then
        if to_tier >= 5 then
            to_tier = 5
...
local function order_all_upgrades(card, rows)
    local upgrades = {}
    local base = card.ability.tower_info and card.ability.tower_info.base
    for path = 1, #rows do
        if path == 2 then
            local tower = G.P_CENTERS['j_bloons_'..JokerTable[base][0][0]]
            table.insert(upgrades, tower)
        end
        for tier = 1, 5 do
            local tower = G.P_CENTERS['j_bloons_'..JokerTable[base][path][tier]]
            table.insert(upgrades, tower)
        end
    end
    return upgrades
...
local function create_UIBox_upgrades(card, rows, args)
    args = args or {}
    ...
    --Grab all jokers from the same tower and put into the pool
    local pool = order_all_upgrades(card, rows)
    ...
    for j = 1, #rows do
        if cards_per_page >= #pool and args.collapse_single_page then
            ...
            {n=G.UIT.R, config={align = "r", padding = 0.07, no_fill = true}, nodes={
...
function BTD.display_upgrades(card)
    ...
    local menu = card.area.config.collection and 'your_collection' or 'exit_overlay_menu'
    G.SETTINGS.paused = true
    G.FUNCS.overlay_menu {
        definition = create_UIBox_upgrades(card, {5,6,5}, {
            back_func = menu,
            no_materialize = true,
            modify_card = function(c, center)
                c.sticker = get_joker_win_sticker(center)
            end,
        })
    }
